...
while not data == 'end':
    ...
    if not ((wizard_one_name in wizards_names_list) and (wizard_two_name in wizards_names_list)):
        print("Cannot place a fight with non-existing wizards!")
    else:
        # FIGHT

        try:
            attacker = list(filter(lambda wiz: wiz.name == wizard_one_name, wizards_list))[0]
            attacked = list(filter(lambda wiz: wiz.name == wizard_two_name, wizards_list))[0]
